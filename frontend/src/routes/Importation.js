...
export default class Importation extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {file: null, checks: {formatOk: false, structureOk: false, dataOk: false}}
        this.onChangeInput = this.onChangeInput.bind(this);
    }

    onChangeInput(file) {
        check(file, (result) => {this.setState({checks: result.checks})});
    }

    render() {
        return (<div>
            <Header icon={mdiTableArrowUp} title="Importation"
                    description="Veuillez choisir le fichier d’étudiants à importer">
            </Header>
            <div className="py-20 h-screen px-2 ">
                <div className="max-w-md mx-auto bg-white rounded-lg overflow-hidden md:max-w-lg shadow-md">
                    <div className="flex flex-col w-full p-3 gap-2">
                        <InputFile file={this.state.file} onFileChange={this.onChangeInput}/>
